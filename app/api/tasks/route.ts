...
export async function GET(request: NextRequest): Promise<NextResponse<ApiResponse<Task[]>>> {
  try {
    ...
    sql += ' ORDER BY COALESCE(due_date, "9999-99-99") ASC, created_at DESC LIMIT ? OFFSET ?'
  ...
  try {
    ...
    await db.execute({
      sql: `INSERT INTO tasks (
        ...
        priority, status, notification_enabled, notification_minutes_before,
      ...
      args: [
        ...
        body.notification_minutes_before || 15,
        now,
        now,
      ],
    })

    // Schedule notification if enabled and has due date/time
    let qstashScheduleId: string | null = null
    if (body.notification_enabled !== false && body.due_date && body.due_time) {
      qstashScheduleId = await scheduleTaskNotification(
        id,
        session.user.id,
        body.due_date,
        body.due_time,
    ...
    const task: Task = {
      ...
      notification_minutes_before: body.notification_minutes_before || 15,
